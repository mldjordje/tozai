...
export async function getPublicPortfolio(
  ...
  try {
    ...
    const en = locale === "en";
    const workText = en
      ? `COALESCE(NULLIF(btrim(title_en), ''), title) AS title,
         COALESCE(NULLIF(btrim(description_en), ''), description) AS description`
      : `title, description`;
    const works = (await sql.query(`
      SELECT id, category_id, ${workText}, client, media_url, media_type, youtube_id, poster_url
    ...
    const categories = (await sql.query(`
      SELECT c.id,
             ${en ? "COALESCE(NULLIF(btrim(c.name_en), ''), c.name)" : "c.name"} AS name,
             c.slug
